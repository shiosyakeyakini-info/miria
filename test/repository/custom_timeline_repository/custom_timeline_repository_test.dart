...
class FakeApiService extends Fake implements ApiService {
  ...
  Future<T> post<T>(String path, Map<String, dynamic> request,
      {bool Function(String key, String? value)? excludeRemoveNullPredicate}) async {
    lastPath = path;
    lastRequest = request;
...
void main() {
  group('CustomTimelineRepository', () {
    ...
    setUp(() {
      ...
      when(misskey.socketConnectionTimeout).thenReturn(const Duration(seconds: 20));
      setting = TabSetting(
    ...
    test('previousLoad uses untilId when available', () async {
      ...
      expect(apiService.lastRequest, {'foo': 'bar', 'untilId': TestData.note1.id});
      expect(count, 1);
